...
class Main(xbmc.Monitor):
        ...
        if 'Other.RPIWSL_VariablePass' in method:
            ...
            if data == 'ScreenStatus:ScreenOff':
                ...
                self._set_properties(
                    self.SENSORINTERFACE.getSensorData(data=data))
        elif 'Other.RPIWSL_SettingsRequest' in method:
            data = json.loads(data)
            self.LW.log(['MONITOR METHOD: %s DATA: %s' %
                        (str(method), str(data))])
            self.SENSORINTERFACE.handlePassback(
                'ReturningSettings', self._scrub_settings())
    ...
    def _init_vars(self):
        ...
        self.LW = Logger(preamble='[WS Lite Monitor]',
    ...
    def _set_property(self, property_name, value=''):
        try:
            ...
            self.LW.log(['Exception: Could not set property %s to value %s' % (
